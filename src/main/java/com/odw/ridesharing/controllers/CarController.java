		...
		if (typeValues_.size() == RuntimeConstants.CREATE_CAR_FORMAT.length) {
			...
			try {
				...
				throw new InvalidCarArgumentsException(
						"Invalid arguments, unable to determine car type. Value passed: " + typeValues_.get(0));
			}
		}

		throw new InvalidCarArgumentsException("Invalid number of arguments for create car. Expected: "
				+ RuntimeConstants.CREATE_CAR_FORMAT.length + ", Actual: " + typeValues_.size());
	}
	...
	public AbstractCar modifyCar(ArrayList<String> typeValues_)
		...
		if (typeValues_.size() == RuntimeConstants.MODIFY_CAR_FORMAT.length) {
			try {
				...
				int _oldYear = _modifiedCar.getYear();
				...
				throw new InvalidCarArgumentsException("Cannot parse CarID/year to integer. Year: " + typeValues_.get(4)
						+ " CarID: " + typeValues_.get(0));
			} catch (NullPointerException e_) {
				throw new CarNotFoundException(
						"Car was not found in the database. CarID Value = " + typeValues_.get(0));
			}
		}

		throw new InvalidCarArgumentsException("Invalid number of arguments for modify car. " + "Expected: "
				+ RuntimeConstants.MODIFY_CAR_FORMAT.length + ", Actual: " + typeValues_.size());
	}
		...
		if (typeValues_.size() == RuntimeConstants.DELETE_CAR_FORMAT.length) {
			try {
				...
				if (carDatabase.get(_carID) != null) {
					...
					throw new CarNotFoundException(
							"Invalid car was not found in the database. CarID Value = " + typeValues_.get(0));
				}
			} catch (NumberFormatException e_) {
				throw new InvalidCarArgumentsException("Cannot parse CarID to integer. CarID: " + typeValues_.get(0));

			}
		}

		throw new InvalidCarArgumentsException("Invalid number of arguments for delete car." + "Expected: "
				+ RuntimeConstants.DELETE_CAR_FORMAT.length + ", Actual: " + typeValues_.size());
	}
